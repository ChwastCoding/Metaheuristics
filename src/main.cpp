...
std::shared_ptr<Solver> getSolver(
    ...
    if (mode == "2Opt") return std::make_shared<TwoOptSolver>(instance);
    if (mode == "2Opt-NN") return std::make_shared<TwoOptSolver>(instance, true);
    if (mode == "NN") {
        return std::make_shared<NearestNeighbour>(instance,
        std::stoi(util::getCmdOption(argc, argv, "--starting-node")));   
    }
    if (mode == "ExtNN") return std::make_shared<ExtNearestNeighbour>(instance);
    if (mode == "kRandom") {
        return std::make_shared<K_RandomSolver>(instance, 
            std::stoi(util::getCmdOption(argc, argv, "--k")));
    }
    else throw std::runtime_error("unknown value of --solver");
}
...
std::shared_ptr<TSPInstance> getInstance(int argc, char** argv) {
    ...
    throw std::runtime_error("invalid parameter combination, can't create instance");
}
...
int main(int argc, char** argv)
{
    if (util::cmdOptionExists(argc, argv, "--graphic")){
        ...
        int iterations = 0;
        while (solver->step() && (iterations < stepLimit || !stepLimit)) iterations ++;
        std::cout << solver->calculateObjectiveFunction() << 
        " " << iterations << std::endl;
    }
    return 0;
}